...
//#define DEBUG
//#undef dev_dbg
//#define dev_dbg(d, f, arg...) dev_err(d, f, ##arg)
...
{
	u32 val = readl(clockman->regs + reg);
					    ...
					    unsigned int fc0_src)
{
	...
	dev_dbg(clockman->dev, "%s: (%s) src %u, result %lu -> %lu KHz\n",
		__func__, clk_name, fc0_src, result, result >> FC0_RESULT_FRAC_SHIFT);
				...
				unsigned long rate, unsigned long parent_rate)
{
	...
	if (clockman->running_on_fpga) {
		dev_dbg(clockman->dev,
			"%s: (%s) rate %lu, parent rate %lu\n",
			__func__, data->name, rate, parent_rate);
		pll_core->cached_rate = rate;
		return 0;
	}
	...
	dev_dbg(clockman->dev,
		"%s: (%s) calc rate %lu, req rate %lu, parent rate %lu, fbdiv_int %u fbdiv_frac %u\n",
		 __func__, data->name, calc_rate, rate, parent_rate,
		 fbdiv_int, fbdiv_frac);
					     ...
					     unsigned long parent_rate)
{
	...
	if (clockman->running_on_fpga) {
		dev_dbg(clockman->dev,
			"%s: (%s) rate %lu, parent rate %lu\n",
			__func__, data->name, pll_core->cached_rate,
			parent_rate);
		return pll_core->cached_rate;
	}
	...
	dev_dbg(clockman->dev,
		"%s: (%s) calc rate %lu, parent rate %lu int %u frac %u\n",
		 __func__, data->name, calc_rate, parent_rate, fbdiv_int, fbdiv_frac);
				   ...
				   unsigned long *parent_rate)
{
	...
	calc_rate = get_pll_core_divider(hw, rate, *parent_rate,
					 &fbdiv_int, &fbdiv_frac);
	dev_dbg(clockman->dev,
		"%s: (%s) calc rate %ld, req rate %lu, parent rate %lu, fbdiv_int %u, fbdiv_frac %u\n",
		 __func__, data->name, calc_rate, rate, *parent_rate,
		 fbdiv_int, fbdiv_frac);
			   ...
			   unsigned long rate, unsigned long parent_rate)
{
	...
	dev_dbg(clockman->dev,
		"%s: (%s) rate %lu, parent rate %lu,  prim div1 %u div2 %u\n",
		 __func__, data->name, rate, parent_rate, prim_div1, prim_div2);
					...
					unsigned long parent_rate)
{
	...
	calc_rate = DIV_ROUND_CLOSEST(parent_rate, prim_div1 * prim_div2);
	dev_dbg(clockman->dev,
		"%s: (%s) rate %lu, parent_rate %lu, prim div1 %u div2 %u\n",
		 __func__, data->name, calc_rate, parent_rate,
		 prim_div1, prim_div2);
			      ...
			      unsigned long *parent_rate)
{
	...
	calc_rate = DIV_ROUND_CLOSEST(*parent_rate, div1 * div2);
	dev_dbg(clockman->dev,
		"%s: (%s) calc rate %ld, rate %lu, parent rate %lu, div1 %u div2 %u\n",
		 __func__, data->name, calc_rate, rate, *parent_rate,
		 div1, div2);
			      ...
			      unsigned long rate, unsigned long parent_rate)
{
	...
	dev_dbg(clockman->dev,
		"%s: (%s) rate %lu parent rate %lu, div %u\n",
		 __func__, data->name, rate, parent_rate, data->fixed_divider);
					   ...
					   unsigned long parent_rate)
{
	...
	dev_dbg(clockman->dev,
		"%s: (%s) parent idx %d, parent rate %lu, div %u\n",
		 __func__, data->name, clk_hw_get_parent_index(hw),
		 parent_rate, data->fixed_divider);
				 ...
				 unsigned long *parent_rate)
{
	...
	const struct rp1_pll_ph_data *data = pll_ph->data;
	struct rp1_clockman *clockman = pll_ph->clockman;
	long calc_rate = *parent_rate / data->fixed_divider;

	dev_dbg(clockman->dev,
		"%s: (%s) calc rate %ld, req rate %lu, parent rate %lu, div %u\n",
		 __func__, data->name, calc_rate, rate, *parent_rate,
		 data->fixed_divider);
				   ...
				   unsigned long parent_rate)
{
	...
	dev_dbg(clockman->dev,
		"%s: (%s) rate %lu, parent_rate %lu, div %u\n",
		 __func__, data->name, rate, parent_rate, div);
						...
						unsigned long parent_rate)
{
	...
	if (clockman->running_on_fpga) {
		dev_dbg(clockman->dev,
			"%s: (%s) rate %lu, parent rate %lu\n",
			__func__, data->name, divider->cached_rate,
			parent_rate);
		return divider->cached_rate;
	}

	calc_rate = clk_divider_ops.recalc_rate(hw, parent_rate);
	dev_dbg(clockman->dev,
		"%s: (%s) calc rate %lu, parent rate %lu\n",
		 __func__, data->name, calc_rate, parent_rate);
				      ...
				      unsigned long *parent_rate)
{
	...
	dev_dbg(clockman->dev,
		"%s: (%s) calc rate %lu, req rate %lu, parent rate %lu\n",
		 __func__, data->name, calc_rate, rate, *parent_rate);
					  ...
					  unsigned long parent_rate)
{
	...
	if (clockman->running_on_fpga) {
		dev_dbg(clockman->dev,
			"%s: (%s) rate %lu, parent rate %lu\n",
			__func__, data->name, clock->cached_rate, parent_rate);
	...
	dev_dbg(clockman->dev,
		"%s: (%s) rate %llu, parent rate %lu, div %llu\n",
		 __func__, data->name, calc_rate, parent_rate, div);
...
static u8 rp1_clock_get_parent(struct clk_hw *hw)
{
	...
	if (!sel) {
		dev_dbg(clockman->dev, "%s: (%s) parent %d is not enabled yet\n",
			__func__, data->name, parent);
					...
					u8 parent)
{
	...
	if (clockman->running_on_fpga) {
		dev_dbg(clockman->dev,
			"%s: (%s) rate %lu, parent rate %lu, parent %x\n",
			__func__, data->name, rate, parent_rate, parent);
	...
	dev_dbg(clockman->dev,
		"%s: (%s) rate %lu, parent idx %d, parent rate %lu, div %u\n",
		 __func__, data->name, rate, parent, parent_rate, div);
					  ...
					  unsigned long *calc_rate)
{
	...
	*calc_rate = tmp;
	...
	dev_dbg(clockman->dev,
		"%s: (%s) calc rate %lu, req rate %lu, parent idx %d, parent rate %lu, div %u\n",
		 __func__, data->name, *calc_rate, rate, parent_idx, *prate, div);
}
				   ...
				   struct clk_rate_request *req)
{
	...
	if ((clk_hw_get_flags(hw) & CLK_SET_RATE_NO_REPARENT)) {
		...
		if (parent != NULL && !is_aux_src(hw, i)) {
			...
			if (calc_rate > 0) {
				dev_dbg(clockman->dev,
					"%s: (%s) force parent %d, calc rate %lu, req rate %lu, prate %lu\n",
	...
	for (i = 0; i < clk_hw_get_num_parents(hw); i++) {
		...
		dev_dbg(clockman->dev,
			"%s: (%s) parent %d, calc rate %lu, req rate %lu, parent rate %lu, best rate %lu\n",
			__func__, data->name, (int)i, calc_rate,
					   ...
					   const void *data)
{
	...
	dev_dbg(clockman->dev, "%s: (%s) parent %s\n",
		 __func__, pll_core_data->name, ref_clock);
				      ...
				      const void *data)
{
	...
	dev_dbg(clockman->dev, "%s: (%s) parent %s\n",
		 __func__, pll_data->name, pll_data->source_pll);
					 ...
					 const void *data)
{
	...
	dev_dbg(clockman->dev,
		"%s: (%s) parent %s\n",
		 __func__, ph_data->name, ph_data->source_pll);
					      ...
					      const void *data)
{
	...
	dev_dbg(clockman->dev,
		"%s: (%s) parent %s\n",
		 __func__, divider_data->name, divider_data->source_pll);
...
/* XXX: don't mess with these clocks
	...
	[RP1_CLK_USBH0_MICROFRAME] = REGISTER_CLK(
				.name = "clk_usbh0_microframe",
				.parents = {"-"},
				.num_std_parents = 1,
				.num_aux_parents = 0,
	...
	[RP1_CLK_USBH1_MICROFRAME] = REGISTER_CLK(
				.name = "clk_usbh1_microframe",
				.parents = {"-"},
				.num_std_parents = 1,
				.num_aux_parents = 0,
	...
	[RP1_CLK_USBH0_SUSPEND] = REGISTER_CLK(
				.name = "clk_usbh0_suspend",
				.parents = {"-"},
				.num_std_parents = 1,
				.num_aux_parents = 0,
	...
	[RP1_CLK_USBH1_SUSPEND] = REGISTER_CLK(
				.name = "clk_usbh1_suspend",
				.parents = {"-"},
				.num_std_parents = 1,
				.num_aux_parents = 0,
...
static int rp1_clk_probe(struct platform_device *pdev)
{
	...
	clockman->running_on_fpga = !!(platform & RP1_PLATFORM_FPGA);
	...
	dev_dbg(clockman->dev, "%s: registered %zd clocks\n", __func__, asize);
