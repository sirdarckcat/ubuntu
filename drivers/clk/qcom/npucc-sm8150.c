...
static struct clk_fixed_factor npu_cc_crc_div = {
	...
	.hw.init = &(struct clk_init_data){
		.name = "npu_cc_crc_div",
		.parent_data = &(const struct clk_parent_data){
				.hw = &npu_cc_pll0_out_even.clkr.hw,
			},
		.num_parents = 1,
		.flags = CLK_SET_RATE_PARENT,
...
static const struct clk_parent_data npu_cc_parent_data_0[] = {
	{ .fw_name = "bi_tcxo" },
	{ .hw = &npu_cc_pll1_out_even.clkr.hw },
	{ .hw = &npu_cc_pll0_out_even.clkr.hw },
...
static struct gdsc npu_core_gdsc = {
	.gdscr = 0x1028,
	.gds_hw_ctrl = 0x0,
	.pd = {
		.name = "npu_core_gdsc",
...
static struct clk_hw *npu_cc_sm8150_hws[] = {
	&npu_cc_crc_div.hw,
};

static struct gdsc *npu_cc_sm8150_gdscs[] = {
	[NPU_CORE_GDSC] = &npu_core_gdsc,
};
...
static const struct qcom_cc_desc npu_cc_sm8150_desc = {
	...
	.gdscs = npu_cc_sm8150_gdscs,
	.num_gdscs = ARRAY_SIZE(npu_cc_sm8150_gdscs),
	.clk_hws = npu_cc_sm8150_hws,
	.num_clk_hws = ARRAY_SIZE(npu_cc_sm8150_hws),
};
...
static int npu_cc_sm8150_probe(struct platform_device *pdev)
{
	...
	/* Enable MND RC */
	regmap_write(regmap, 0x1010, 0x15010);
	regmap_write(regmap, 0x100c, 0x800000);
