...
#define is_fe_node(id) ((id) >= cfe->sensor->entity.num_pads)
#define is_csi2_node(id) (!is_fe_node(id))
#define is_image_output_node(id)                                               \
	(node_desc[id].buf_type == V4L2_BUF_TYPE_VIDEO_CAPTURE)
#define is_meta_output_node(id)                                                \
	(node_desc[id].buf_type == V4L2_BUF_TYPE_META_CAPTURE)
#define is_meta_input_node(id)                                                 \
	(node_desc[id].buf_type == V4L2_BUF_TYPE_META_OUTPUT)
#define is_meta_node(id) (is_meta_output_node(id) || is_meta_input_node(id))
...
struct cfe_device {
	...
	/* Track the number of open nodes */
	atomic_t open_count;
...
{
	u32 val = readl(cfe->mipi_cfg_base + offset);
	return val;
}
...
static inline int cfe_runtime_get(struct cfe_device *cfe)
{
	return pm_runtime_get_sync(&cfe->pdev->dev);
}

static inline void cfe_runtime_put(struct cfe_device *cfe)
{
...
static int format_show(struct seq_file *s, void *data)
{
	...
	for (i = 0; i < NUM_NODES; i++) {
		...
		seq_printf(s, "\nNode %d (%s) state: 0x%lx\n", i,
			   node_desc[i].name, state);

		if (is_image_output_node(i))
...
static const struct cfe_fmt *find_format_by_code(u32 code)
{
	unsigned int i;
				    ...
				    struct v4l2_format *f)
{
	...
	cfe_dbg("%s: " V4L2_FOURCC_CONV " size: %dx%d bpl:%d img_size:%d\n",
...
static void eof_isr_handler(struct cfe_node *node)
{
	 ...
	 *
	 * Account for nodes that may not have been supplied any
	 * buffers, in which case, it would not have picked up
	 * a FS either.
	 */
	if (test_all_nodes(cfe, NODE_STREAMING + FS_INT, FE_INT)) {
		cfe->sequence++;
				...
				struct v4l2_fmtdesc *f)
{
	...
	for (i = 0, j = 0; i < ARRAY_SIZE(formats); i++) {
		...
		if (is_fe_node(node->id) && !formats[i].remap[CFE_REMAP_16BIT])
			continue;
		...
		if (j == f->index) {
			f->pixelformat =
				!is_fe_node(node->id) ? formats[i].fourcc
					: formats[i].remap[CFE_REMAP_16BIT];
			f->type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
			return 0;
		}
		j++;

		for (k = 0; k < CFE_NUM_REMAP; k++) {
			if (formats[i].remap[k]) {
				if (j == f->index) {
					f->pixelformat = formats[i].remap[k];
					f->type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
					return 0;
				}
				j++;
...
static int try_fmt_vid_cap(struct cfe_node *node, struct v4l2_format *f)
{
	...
	cfe_dbg("%s: [%s] %dx%d, V4L2 pix " V4L2_FOURCC_CONV "\n",
		__func__, node_desc[node->id].name,
		f->fmt.pix.width, f->fmt.pix.height,
	...
	if (!fmt) {
		cfe_err("Unable to find appropriate pixelformat\n");
		return -EINVAL;
	}

	if (is_fe_node(node->id) && fmt->remap[CFE_REMAP_16BIT]) {
			     ...
			     struct v4l2_format *f)
{
	...
	cfe_dbg("%s: Set %dx%d, V4L2 pix " V4L2_FOURCC_CONV "\n", __func__,
			     ...
			     struct v4l2_fmtdesc *f)
{
	...
	if (!is_meta_node(node->id) || f->index != 0)
		return -EINVAL;
			   ...
			   struct device *alloc_devs[])
{
	...
	struct cfe_device *cfe = node->cfe;
	unsigned int size = is_image_output_node(node->id) ?
					  node->fmt.fmt.pix.sizeimage :
					  node->fmt.fmt.meta.buffersize;
...
static int cfe_start_streaming(struct vb2_queue *vq, unsigned int count)
{
	...
	if (!cfe->csi2.active_data_lanes)
		cfe->csi2.active_data_lanes = cfe->csi2.num_lanes;
	if (cfe->csi2.active_data_lanes > cfe->csi2.num_lanes) {
			       ...
			       struct v4l2_frmsizeenum *fsize)
{
	...
	cfe_dbg("%s: index: %d code: %x W:[%d,%d] H:[%d,%d]\n",
...
static int cfe_log_status(struct file *file, void *fh)
{
	struct cfe_node *node = video_drvdata(file);
	struct cfe_device *cfe = node->cfe;

	/* status for sub devices */
	v4l2_device_call_all(&cfe->v4l2_dev, 0, core, log_status);

	cfe_info("-----Receiver status-----\n");
	cfe_info("V4L2 width/height:   %ux%u\n", node->fmt.fmt.pix.width,
		 node->fmt.fmt.pix.height);
	cfe_info("V4L2 format:         %08x\n",
		 node->fmt.fmt.pix.pixelformat);
...
static int cfe_v4l2_open(struct file *file)
{
	struct cfe_node *node = video_drvdata(file);
	struct cfe_device *cfe = node->cfe;
	int ret;
	...
	cfe_dbg("%s: [%s] id %d\n", __func__,
		node_desc[node->id].name, node->id);

	mutex_lock(&node->lock);

	ret = v4l2_fh_open(file);
	if (ret) {
		cfe_err("v4l2_fh_open failed\n");
		goto unlock;
	}

	if (atomic_inc_return(&cfe->open_count) == 1) {
		ret = v4l2_subdev_call(cfe->sensor, core, s_power, 1);
		if (ret < 0 && ret != -ENOIOCTLCMD) {
...
static int cfe_v4l2_release(struct file *file)
{
	struct cfe_node *node = video_drvdata(file);
	struct cfe_device *cfe = node->cfe;
	struct v4l2_subdev *sd = cfe->sensor;
	int ret;
	...
	if (!atomic_dec_if_positive(&cfe->open_count)) {
...
static const struct v4l2_ioctl_ops cfe_ioctl_ops = {
	...
	.vidioc_log_status = cfe_log_status,
...
static int cfe_video_link_validate(struct media_link *link)
{
	...
	ret = cfe_subdev_link_get_format(link->source, &source_fmt);
	if (ret < 0)
		return 0;
	...
	if (is_image_output_node(node->id)) {
		...
		if (source_fmt.format.width * source_fmt.format.height !=
							meta_fmt->buffersize ||
				 ...
				 unsigned int notification)
{
	...
	cfe_dbg("%s: %s[%d] -> %s[%d] 0x%x", __func__,
		link->source->entity->name, link->source->index,
	...
	if (is_fe_enabled(cfe))
		cfe_info("%s: Found CSI2:%d -> FE:0 link\n", __func__,
			 cfe->fe_csi2_channel);
	else
		cfe_info("%s: Unable to find CSI2:x -> FE:0 link\n", __func__);
...
static int register_node(struct cfe_device *cfe, int id)
{
	...
	vdev->vfl_dir = (is_image_output_node(id) || is_meta_output_node(id))
	...
	cfe_info("Registered [%s] node id %d successfully as /dev/video%d\n",
...
static void unregister_nodes(struct cfe_device *cfe)
{
	unsigned int i;
...
static int link_node_pads(struct cfe_device *cfe)
{
	unsigned int i;
	int ret;
...
static int cfe_probe_complete(struct cfe_device *cfe)
{
	...
	for (i = 0; i < NUM_NODES; i++) {
		ret = register_node(cfe, i);
		if (ret) {
			cfe_err("Unable to register video node %d.\n", i);
			goto unregister;
		}
	}

	ret = link_node_pads(cfe);
	if (ret) {
...
static int of_cfe_connect_subdevs(struct cfe_device *cfe)
{
	...
	if (ep.bus_type != V4L2_MBUS_CSI2_DPHY) {
		cfe_err("endpoint node type != CFE\n");
		return -EINVAL;
	}
	...
	cfe_dbg("subdevice %pOF: %u data lanes, flags=0x%08x, multipacket_line=%d\n",
		sensor_node, cfe->csi2.num_lanes, cfe->csi2.bus_flags,
		cfe->csi2.multipacket_line);
...
static int cfe_probe(struct platform_device *pdev)
{
	...
	cfe->csi2.host_base = devm_platform_ioremap_resource(pdev, 1);
	if (IS_ERR(cfe->csi2.base)) {
		dev_err(&pdev->dev, "Failed to get host io block\n");
		ret = PTR_ERR(cfe->csi2.host_base);
		goto err_cfe_put;
	}
	...
	mipicfg_inte = cfg_reg_read(cfe, INTE);
	mipicfg_inte |= INT_CSI_DMA + INT_PISP_FE;
