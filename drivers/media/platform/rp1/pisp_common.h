...
enum pisp_axi_flags {
	PISP_AXI_FLAG_ALIGN =
		128, /* round down bursts to end at a 32-byte boundary, to align following bursts */
	PISP_AXI_FLAG_PAD =
		64, /* for FE writer: force WSTRB high, to pad output to 16-byte boundary */
	PISP_AXI_FLAG_PANIC =
		32 /* for FE writer: Use Output FIFO level to trigger "panic" */
};
