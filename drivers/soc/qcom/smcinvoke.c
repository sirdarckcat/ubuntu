...
/*
 * __size_add saturates at SIZE_MAX. If integer overflow is detected,
 * this function would return SIZE_MAX otherwise normal a+b is returned.
 */
static inline size_t __size_add(size_t a, size_t b)
{
...
{
	return __size_add(a, pad_size(a, b));
}
					...
					const union smcinvoke_arg *args_buf)
{
	...
	while (i < OBJECT_COUNTS_NUM_buffers(req->counts))
		total_size = __size_add(total_size,
