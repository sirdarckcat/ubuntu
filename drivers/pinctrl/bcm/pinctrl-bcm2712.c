...
#define GPIO_REGS(n, r, b) \
	[n] = { GPIO_PINMUX_0*8 + (n)*4, (GPIO_PADCTRL_0 + (r)*4)*8 + (b)*2 }

#define EMMC_REGS(n, r, b) \
	[n] = { 0, (GPIO_PADCTRL_0 + (r)*4)*8 + (b)*2 }

#define AGPIO_REGS(n, pr, pb) \
	[n] = { AON_GPIO_PINMUX_3*8 + (n)*4, (AON_GPIO_PADCTRL_0 + (pr)*4)*8 + (pb)*2 }
...
static const struct bcm2712_pin_funcs bcm2712_d0_gpio_pin_funcs[] = {
	...
	PIN(5, _, _, _, _, _, _, _, _),
	PIN(6, _, _, _, _, _, _, _, _),
	PIN(7, _, _, _, _, _, _, _, _),
	PIN(8, _, _, _, _, _, _, _, _),
	PIN(9, _, _, _, _, _, _, _, _),
	...
	PIN(16, _, _, _, _, _, _, _, _),
	PIN(17, _, _, _, _, _, _, _, _),
