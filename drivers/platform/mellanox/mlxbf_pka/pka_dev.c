...
struct pka_dev_gbl_config_t pka_gbl_config;

/* Global PKA shim resource info table */
static
struct pka_dev_gbl_shim_res_info_t pka_gbl_res_tbl[PKA_MAX_NUM_IO_BLOCKS];
...
static uint64_t pka_dev_get_word_offset(uint64_t mem_base,
					uint64_t word_addr,
					uint64_t mem_size)
{
...
static int pka_dev_add_resource(struct pka_dev_res_t *res_ptr,
				uint32_t              shim_idx)
{
	uint8_t res_cnt;
...
static int pka_dev_put_resource(struct pka_dev_res_t *res,  uint32_t shim_idx)
{
	struct pka_dev_res_t *res_ptr;
	uint8_t               res_idx;
...
static int pka_dev_set_resource_config(struct pka_dev_shim_t *shim,
				       struct pka_dev_res_t *res_ptr,
				       uint64_t              res_base,
				       uint64_t              res_size,
				       uint64_t              res_type,
				       char                 *res_name)
{
	...
	res_ptr->ioaddr  = pka_dev_get_resource_ioaddr(res_ptr->base,
						       shim->shim_id);
	if (!res_ptr->ioaddr) {
		if (!request_mem_region(res_ptr->base,
					res_ptr->size,
					res_ptr->name))	{
...
static void pka_dev_unset_resource_config(struct pka_dev_shim_t *shim,
					  struct pka_dev_res_t  *res_ptr)
{
	int ret = -EBUSY;
 ...
 */
static int pka_dev_init_ring(struct pka_dev_ring_t *ring,
			     uint32_t               ring_id,
			     struct pka_dev_shim_t *shim)
{
	int ret = 0;

	struct pka_dev_res_t *ring_info_words_ptr;
	struct pka_dev_res_t *ring_counters_ptr;
	struct pka_dev_res_t *ring_window_ram_ptr;
	...
	ring->ring_info = kzalloc(sizeof(struct pka_dev_hw_ring_info_t),
				  GFP_KERNEL);
	if (!ring->ring_info) {
...
static int pka_dev_release_ring(struct pka_dev_ring_t *ring)
{
	int ret = 0;
 ...
 */
static int pka_dev_partition_mem(struct pka_dev_ring_t *ring)
{
	int ret = 0;
 ...
 */
static int pka_dev_write_ring_info(struct pka_dev_res_t *buffer_ram_ptr,
				   uint8_t               ring_id,
 ...
 */
static int pka_dev_set_ring_info(struct pka_dev_ring_t *ring)
{
	int ret = 0;

	struct pka_dev_shim_t         *shim;
	struct pka_dev_hw_ring_info_t *ring_info;
	struct pka_dev_res_t          *buffer_ram_ptr;
	uint32_t ring_cmmd_base_val;
	uint32_t ring_rslt_base_val;
	uint32_t ring_size_type_val;
	uint8_t  ring_id;
	...
	ret = pka_dev_write_ring_info(buffer_ram_ptr,
				      ring_id,
				      ring_cmmd_base_val,
				      ring_rslt_base_val,
				      ring_size_type_val);
	if (ret) {
		PKA_ERROR(PKA_DEV, "failed to write ring information\n");
		return ret;
	}
 ...
 * It returns 0 on success, a negative error code on failure.
 */
static int pka_dev_create_shim(struct pka_dev_shim_t *shim,
			       uint32_t               shim_id,
			       uint64_t               shim_base,
			       uint64_t               shim_size,
			       uint8_t                split)
{
	...
	shim->rings = kzalloc(sizeof(struct pka_dev_ring_t) * shim->rings_num,
			      GFP_KERNEL);
	if (!shim->rings) {
		PKA_ERROR(PKA_DEV, "unable to kmalloc\n");
		kfree(shim->rings);
	...
	ret = pka_dev_set_resource_config(shim,
	...
	ret = pka_dev_set_resource_config(shim,
					  &shim->resources.master_prog_ram,
...
static int pka_dev_delete_shim(struct pka_dev_shim_t *shim)
{
	int ret = 0;
	struct pka_dev_res_t *res_buffer_ram, *res_master_prog_ram;
	struct pka_dev_res_t *res_master_seq_ctrl, *res_aic_csr, *res_trng_csr;
...
static int pka_dev_config_aic_interrupts(struct pka_dev_res_t *aic_csr_ptr)
{
	int ret = 0;
...
static int pka_dev_load_image(struct pka_dev_res_t *res_ptr,
			      ...
			      uint32_t              size)
{
	...
	for (i = 0; i < size; i++) {
		data_rd = pka_dev_io_read(res_ptr->ioaddr,
					  i * BYTES_PER_DOUBLE_WORD);
		if (data_rd != (uint64_t) data_buf[i]) {
			mismatches += 1;
			PKA_DEBUG(PKA_DEV,
				  "error while loading image: addr:0x%llx "
				  "expected data: 0x%x actual data: 0x%llx\n",
...
static int
pka_dev_config_master_seq_controller(struct pka_dev_shim_t *shim,
				     struct pka_dev_res_t *master_seq_ctrl_ptr)
{
	struct pka_dev_res_t  *aic_csr_ptr, *master_prog_ram;
	...
	/*
	 * Poll for 'pka_master_irq' bit in AIC_ENABLED_STAT register to
	 * indicate sequencer is initialized
	 */
	...
	if (status_bits != PKA_MASTER_SEQ_CTRL_STATUS_BYTE) {
		 ...
		 */
		if ((status_bits >>
		     (PKA_MASTER_SEQ_CTRL_MASTER_IRQ_BIT - 1)) == 1) {
			PKA_ERROR(PKA_DEV,
...
static int pka_dev_config_ring_options(struct pka_dev_res_t *buffer_ram_ptr,
				       uint32_t              rings_num,
				       uint8_t               ring_priority)
{
	...
	/*
	 * Write PKA_RING_OPTIONS control word located in the PKA_BUFFER_RAM.
	 * The value of this word is determined by the PKA I/O block (Shim).
...
static int pka_dev_config_trng_clk(struct pka_dev_res_t *aic_csr_ptr)
{
	int ret = 0;
...
/* Configure the TRNG. */
static int pka_dev_config_trng(struct pka_dev_res_t *aic_csr_ptr,
			       struct pka_dev_res_t *trng_csr_ptr)
{
	int ret = 0;
 ...
 */
static int pka_dev_init_shim(struct pka_dev_shim_t *shim)
{
...
static int pka_dev_release_shim(struct pka_dev_shim_t *shim)
{
	int ret = 0;
	...
	if (shim->status != PKA_SHIM_STATUS_INITIALIZED &&
	    shim->status != PKA_SHIM_STATUS_STOPPED) {
		PKA_ERROR(PKA_DEV,
			  "PKA device must be initialized or stopped\n");
		return -EPERM;
	}
...
static struct pka_dev_ring_t *__pka_dev_register_ring(uint32_t ring_id,
						      uint32_t shim_id)
{
	struct pka_dev_shim_t *shim;
	...
	ring = kzalloc(sizeof(struct pka_dev_ring_t), GFP_KERNEL);
	if (!ring)
		return ring;
...
struct pka_dev_ring_t *pka_dev_register_ring(uint32_t ring_id, uint32_t shim_id)
{
	struct pka_dev_ring_t *ring;
...
static int __pka_dev_unregister_ring(struct pka_dev_ring_t *ring)
{
	int ret;
...
static struct pka_dev_shim_t *__pka_dev_register_shim(uint32_t shim_id,
						      uint64_t shim_base,
						      uint64_t shim_size)
{
	struct pka_dev_shim_t *shim;
	...
	shim = kzalloc(sizeof(struct pka_dev_shim_t), GFP_KERNEL);
	if (!shim)
		return shim;

	/*
	 * Shim state MUST be set to undefined before calling
	 * 'pka_dev_create_shim' function
	 */
	shim->status = PKA_SHIM_STATUS_UNDEFINED;
...
static int __pka_dev_unregister_shim(struct pka_dev_shim_t *shim)
{
	int ret = 0;
...
static bool pka_dev_trng_shutdown_oflo(struct pka_dev_res_t *trng_csr_ptr,
				       uint64_t             *err_cycle)
{
	...
	if (csr_reg_value & PKA_TRNG_STATUS_SHUTDOWN_OFLO) {
		...
		if (fro_stopped_mask) {
			csr_reg_off      =
				pka_dev_get_register_offset(csr_reg_base,
							   TRNG_FROENABLE_ADDR);
			fro_enabled_mask =
				pka_dev_io_read(csr_reg_ptr, csr_reg_off);

			csr_reg_off      =
				pka_dev_get_register_offset(csr_reg_base,
							   TRNG_FRODETUNE_ADDR);
			pka_dev_io_write(csr_reg_ptr,
					 csr_reg_off,
					 fro_stopped_mask);
...
int pka_dev_trng_read(struct pka_dev_shim_t *shim, uint32_t *data, uint32_t cnt)
{
	int ret = 0;
	...
	for (data_idx = 0; data_idx < word_cnt; data_idx++) {
		...
		csr_reg_off =
			pka_dev_get_register_offset(csr_reg_base,
						    TRNG_STATUS_ADDR);
		while (trng_ready == 0)	{
			csr_reg_value = pka_dev_io_read(csr_reg_ptr,
							csr_reg_off);
