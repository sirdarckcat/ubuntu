			    ...
			    struct drm_printer *p)
{
	...
	if (kmem_cache_get_slabinfo(cache, &info))
		return;

	drm_printf(p, "%s: { ", name);
	drm_printf(p, "slab: %s, ", cache->name);
	drm_printf(p, "active: %lu, ", info.active_objs);
	drm_printf(p, "object-size: %u, ", cache->object_size);
	drm_printf(p, "alignment: %u, ", cache->align);
	drm_printf(p, "total: %lu, ",
		   info.active_slabs * info.objects_per_slab * cache->size);
	drm_printf(p, "}\n");
}
