...
const char *stringify_granularity(int val)
{
	switch (val) {
	case 0:
		return "128KB Region with 4KB sub-granularity";
	case 1:
		return "2MB Region with 64KB sub-granularity";
	case 2:
		return "16MB Region with 512KB sub-granularity";
	case 3:
		return "64MB contiguous region with 2M sub-granularity";
	default:
		return "undefined";
	}
}
...
{
	struct access_counter_desc *desc;
	struct intel_gt *gt = guc_to_gt(guc);

	if (unlikely(len != 4))
		return -EPROTO;
	desc = (struct access_counter_desc *)payload;
	trace_intel_access_counter(gt, desc);
	pr_info("Access counter request\n"
		"\tType: %s\n"
		"\tGranularity: %s\n"
		"\tSub_Granularity Vector: 0x%08lx\n"
		"\tASID: %ld\n"
		"\tEngineClass: %ld\n"
		"\tEngineInstance: %ld\n"
		"\tVA Range: 0x%08lx_%08lx\n",
		FIELD_GET(ACCESS_COUNTER_TYPE, desc->dw0) ? "AC_NTFY_VAL" : "AC_TRIG_VAL",
		stringify_granularity(FIELD_GET(ACCESS_COUNTER_GRANULARITY, desc->dw2)),
		(FIELD_GET(ACCESS_COUNTER_SUBG_HI, desc->dw1) << 31 | FIELD_GET(ACCESS_COUNTER_SUBG_LO, desc->dw0)),
		FIELD_GET(ACCESS_COUNTER_ASID, desc->dw1),
		FIELD_GET(ACCESS_COUNTER_ENG_CLASS, desc->dw1),
		FIELD_GET(ACCESS_COUNTER_ENG_INSTANCE, desc->dw1),
		FIELD_GET(ACCESS_COUNTER_VIRTUAL_ADDR_RANGE_HI, desc->dw3),
