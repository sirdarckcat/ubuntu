...
static void pf_clear_flr_in_progress(struct intel_iov* iov, u32 vfid);
...
void intel_iov_state_reset(struct intel_iov *iov)
{
	...
	for (n = 0; n < 1 + pf_get_totalvfs(iov); n++) {
		pf_clear_flr_in_progress(iov, n);
	}
}
...
static bool pf_mark_flr_in_progress(struct intel_iov* iov, u32 vfid)
{
	GEM_BUG_ON(!intel_iov_is_pf(iov));
	GEM_BUG_ON(vfid > pf_get_totalvfs(iov));

	return !atomic_cmpxchg(&iov->pf.state.data[vfid].flr_in_progress, 0, 1);
}
...
static void pf_clear_flr_in_progress(struct intel_iov* iov, u32 vfid)
{
	...
	atomic_set(&iov->pf.state.data[vfid].flr_in_progress, 0);
}
...
static void pf_trigger_vf_flr_start(struct intel_iov *iov, u32 vfid)
{
	...
	if (unlikely(!pf_mark_flr_in_progress(iov, vfid))) {
		IOV_DEBUG(iov, "VF%u FLR already in progress\n", vfid);
		return;
	}
	...
	if (unlikely(ret < 0)) {
		IOV_ERROR(iov, "Failed to start FLR for VF%u (%pe)\n",
			  vfid, ERR_PTR(ret));
		pf_clear_flr_in_progress(iov, vfid);
		return;
	}
}
...
static void pf_handle_vf_flr(struct intel_iov *iov, u32 vfid)
{
	...
	if (!iov_is_root(iov)) {
		IOV_ERROR(iov, "Ignoring VF%u FLR\n", vfid);
		return;
	}
	...
	for_each_gt(iov_to_i915(iov), gtid, gt)
		pf_trigger_vf_flr_start(&gt->iov, vfid);
}
