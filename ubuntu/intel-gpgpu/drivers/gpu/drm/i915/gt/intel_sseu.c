		   ...
		   const u8 *subslice_mask, u8 slice)
{
	...
	for (i = 0; i < sseu->ss_stride; i++)
		mask |= (u64)subslice_mask[offset + i] << i * BITS_PER_BYTE;

	return mask;
}

u64 intel_sseu_get_subslices(const struct sseu_dev_info *sseu, u8 slice)
{
...
u64 intel_sseu_get_compute_subslices(const struct sseu_dev_info *sseu)
{
	return sseu_get_subslices(sseu, sseu->compute_subslice_mask, 0);
}

void intel_sseu_set_subslices(struct sseu_dev_info *sseu, int slice,
			      u8 *subslice_mask, u64 ss_mask)
{
...
{
	return hweight64(intel_sseu_get_subslices(sseu, slice));
}
...
static u64 get_ss_stride_mask(struct sseu_dev_info *sseu, u8 s, u64 ss_en)
{
	u64 ss_mask;

	ss_mask = ss_en >> (s * sseu->max_subslices);
	ss_mask &= GENMASK_ULL(sseu->max_subslices - 1, 0);

	return ss_mask;
}

static void gen11_compute_sseu_info(struct sseu_dev_info *sseu, u8 s_en,
				    u64 g_ss_en, u64 c_ss_en, u16 eu_en)
{
	int s, ss;
...
static void gen12_sseu_info_init(struct intel_gt *gt)
{
	...
	if (IS_PONTEVECCHIO(gt->i915))
		intel_sseu_set_info(sseu, 1, 64, 8);
	else if (IS_DG2(gt->i915) || IS_XEHPSDV(gt->i915))
	...
	/*
	 * On PVC each bit is associated with one EU. On other platforms
	 * each bit is associated with a pair of EUs.
	 */
	if (GRAPHICS_VER_FULL(gt->i915) >= IP_VER(12, 50))
...
void intel_sseu_dump(const struct sseu_dev_info *sseu, struct drm_printer *p)
{
	...
	for (s = 0; s < sseu->max_slices; s++) {
		drm_printf(p, "slice%d: %u subslices, mask=%016llx\n",
				    ...
				    struct drm_printer *p)
{
	...
	for (s = 0; s < sseu->max_slices; s++) {
		drm_printf(p, "slice%d: %u subslice(s) (0x%016llx):\n",
			   s, intel_sseu_subslices_per_slice(sseu, s),
			   intel_sseu_get_subslices(sseu, s));
