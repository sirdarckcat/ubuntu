		     ...
		     void *data)
{
	...
	xa_lock(&vma->vm->file->client->uuids_xa);
	uuid = xa_load(&vma->vm->file->client->uuids_xa, handle);
	if (!uuid) {
		xa_unlock(&vma->vm->file->client->uuids_xa);
		kfree(metadata);
		return -ENOENT;
	}
	metadata->uuid = uuid;
				   ...
				   bool debug_destroy)
{
	...
	if (debug_destroy)
		i915_debugger_vm_bind_destroy(vm->file->client, vma);
...
put_vma:
	if (ret)
		...
		i915_debugger_vm_bind_create(vm->file->client, vma, va);
unlock_vm:
	i915_gem_vm_bind_unlock(vm);
	if (ret && vma) {
