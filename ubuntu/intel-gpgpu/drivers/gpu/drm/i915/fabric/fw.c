...
#define I915_SPI_DRIVER_NAME	"i915-spi"
 ...
 */
#if IS_ENABLED(CPTCFG_IAF_DEBUG_ALTERNATE_FW_LOAD)
static char *psc_file_override = "i915/default_iaf.pscbin";
...
#else
static const char * const psc_file_override;
#endif
...
static u32 firmware_roption = 0x201;
module_param(firmware_roption, uint, 0600);
MODULE_PARM_DESC(firmware_roption, "Set the firmware roption debug register (default: 0x201)");
#else
static const u32 firmware_roption = 0x201;
#endif
...
static void init_opcodes(void)
{
	...
	__set_bit(MBOX_OP_CODE_QSFP_MGR_FAULT_TRAP_NOTIFICATION,
		  required_runtime_opcodes);
	__set_bit(MBOX_OP_CODE_QSFP_MGR_FAULT_TRAP_ACKNOWLEDGE,
		  required_runtime_opcodes);
...
{
	return strcmp(dev_driver_string(dev), I915_SPI_DRIVER_NAME) == 0;
}
...
{
	/* normally checked by init_fw, check here also in case load_fw or start_fw failed */
	wait_for_completion(&dev->psc.done);
...
static void fetch_platform_specific_config(struct fdev *dev)
{
	...
	if (fetching_psc_from_spi)
		...
		} while (dev->psc.err == -ENODEV && cnt++ < MAX_200MS_RETRIES &&
			 !wait_for_completion_timeout(&dev->psc.abort, msecs_to_jiffies(200)));
