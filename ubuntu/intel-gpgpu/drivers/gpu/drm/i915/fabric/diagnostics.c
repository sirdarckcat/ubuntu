...
static const char * const lcb_err_info_names[] = {
	"TOTAL_CRC_ERR",
	"CRC_ERR_LN0",
	"CRC_ERR_LN1",
	"CRC_ERR_LN2",
...
#define LCB_ERR_INFO_VALUES ARRAY_SIZE(lcb_err_info_names)
...
struct lcb_counters_regs_data {
	DECLARE_MBDB_OP_PORT_STATUS_GET_RSP(regs_op, LCB_ERR_INFO_VALUES + LCB_PRF_VALUES);
} __packed;
...
static int lcb_counters_open(struct inode *inode, struct file *file)
{
	...
	for (i = 0; i < LCB_ERR_INFO_VALUES; i++)
	...
	for (i = 0; i < LCB_PRF_VALUES; i++)
		if (lcb_prf_names[i])
			print_diag(buf, &buf_offset, buf_size, LCB_COUNTERS_FMT, lcb_prf_names[i],
				   regs.regs[i + LCB_ERR_INFO_VALUES]);
...
#ifdef SHIFT_LEFT
	/* if not provided by a simulation environment, provide a set of valid values */
	if (!value)
		value = FIELD_PREP(PEER_TX_ID_LN1, 1) |
			FIELD_PREP(PEER_TX_ID_LN2, 2) |
			FIELD_PREP(PEER_TX_ID_LN3, 3);
#endif
